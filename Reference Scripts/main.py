...
# Kafka topic config with environment fallback
...
def generate_weather_data(device_id, timestamp, location):
    return {
        ...
        'airQualityIndex': int(random.uniform(0, 500))
    }
...
def simulation_journey(producer, device_id):
    while True:
        vehicle_data = generate_vehicle_data(device_id)
        gps_data = generate_gps_data(device_id, vehicle_data['timestamp'])
        traffic_data = generate_traffic_camera_data(device_id, vehicle_data['timestamp'], vehicle_data['location'], 'Camera1')
...
if __name__ == '__main__':
    ...
    try:
        simulation_journey(producer, 'Vehicle-CodewithYu-123')
